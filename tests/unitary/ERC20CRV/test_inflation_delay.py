WEEK = 86400 * 7
YEAR = 365 * 86400

def test_start_epoch_time(accounts, chain, token):
    creation_time = token.start_epoch_time()
    assert creation_time == token.tx.timestamp + 86400 + WEEK - YEAR

    chain.sleep(86401)
    token.update_boost_mining_parameters({'from': accounts[0]})
    assert token.start_epoch_time() == creation_time
    chain.mine()
    chain.sleep(WEEK)
    token.update_mining_parameters({'from': accounts[0]})


    assert token.start_epoch_time() == creation_time + YEAR

def test_boost_rate(accounts, chain, token):
    assert token.rate() == 0

    chain.sleep(86401)
    token.update_boost_mining_parameters({'from': accounts[0]})

    assert token.rate() > 0

def test_rate(accounts, chain, token):
    assert token.rate() == 0

    chain.sleep(86401)  # Inflation Delay
    token.update_boost_mining_parameters({'from': accounts[0]})
    chain.mine()
    chain.sleep(WEEK)   # Boost period
    token.update_mining_parameters({'from': accounts[0]})

    assert token.rate() > 0

def test_mining_epoch(accounts, chain, token):
    assert token.mining_epoch() == -1

    chain.sleep(86401)
    token.update_boost_mining_parameters({'from': accounts[0]})
    assert token.mining_epoch() == -1
    chain.mine()
    chain.sleep(WEEK)
    token.update_mining_parameters({'from': accounts[0]})
    assert token.mining_epoch() == 0


def test_available_supply(accounts, chain, token):
    assert token.available_supply() == 50_000_000 * 10**18

    chain.sleep(86401)
    token.update_boost_mining_parameters({'from': accounts[0]})
    chain.mine()
    chain.sleep(WEEK)
    token.update_mining_parameters({'from': accounts[0]})
    chain.mine()
    assert token.available_supply() > 340_000_000 * 10**18
