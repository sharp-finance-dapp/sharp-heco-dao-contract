...
def initial_setup(accounts, chain, token):
    chain.sleep(86401)
...
def test_available_supply(chain, token):
    ...
    initial_supply = token.available_supply() # after boost period, there are unminted tokens
    rate = token.rate()
    chain.sleep(WEEK)
    chain.mine()
...
def test_overmint(accounts, chain, token):
    token.set_minter(accounts[0], {'from': accounts[0]})
    mintable = token.available_supply()
    creation_time = token.start_epoch_time()
    rate = token.rate()
    chain.sleep(WEEK)

    with brownie.reverts("dev: exceeds allowable mint amount"):
        token.mint(accounts[1], mintable + (
            chain.time()-creation_time+2) * rate, {'from': accounts[0]})
