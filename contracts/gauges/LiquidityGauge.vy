...
BOOST_WARMUP: constant(uint256) = 1 * 86400 #
...
future_epoch_time: public(uint256) # next year
...
def _checkpoint(addr: address):
    ...
    if block.timestamp > _period_time:
        prev_week_time: uint256 = _period_time ## 上一次check的时间
        week_time: uint256 = min((_period_time + WEEK) / WEEK * WEEK, block.timestamp) # 需要check到的时间, 一般是后者 min(1611792000, 1611286932)

        for i in range(500):
            dt: uint256 = week_time - prev_week_time
            w: uint256 = Controller(_controller).gauge_relative_weight(self, prev_week_time / WEEK * WEEK) # time: 1611187200 w: 
